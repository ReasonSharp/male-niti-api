...
router.post('/', rateLimiter.strict, asyncHandler(async (req, res) => {
 ...
 // Only authenticated callers get their image persisted to disk.
 if (req.auth) {
  try {
   ...
   await wf.writeFile(path.resolve(outputPath), buf);
   console.log('image written');
  } catch (writeError) {
   console.error('Error saving image to disk: ', writeError.message);
  }
 }

 // Send the image as a response regardless of whether it was saved to disk
