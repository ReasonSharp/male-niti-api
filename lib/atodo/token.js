...
function toUser(row) {
 return {
  id: row.id,
  email: row.email,
  // Only while its verification link is still valid -- an expired pending
  // change is as good as gone (POST /auth/verify-email-change rejects it).
  pendingEmail: row.pending_email && row.pending_email_expires_at && new Date(row.pending_email_expires_at).getTime() > Date.now() ? row.pending_email : null,
  ...
  // Whether POST /subscriptions/trial would succeed -- see its own comment.
  trialAvailable: !row.subscription_plan && !row.trial_ineligible,
 };
}
