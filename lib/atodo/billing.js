...
const { getFiscalConfig, skippingReceipts } = require('./payments');
const { issueReceipt } = require('./fiscal/receipts');
const { notifyOwner } = require('./ownerAlerts');
...
// re-delivers the event later rather than it being lost -- except in Stripe
// test mode without a test-service certificate, where no receipt is issued
// at all (see lib/atodo/payments.js).
async function handleInvoicePaid(invoice) {
 ...
 if (skippingReceipts()) {
  console.log(`[atodo billing] invoice ${invoice.id} paid (Stripe test mode) -- no receipt issued`);
  return;
 }
...
// The subscription invoice a PaymentIntent paid, if any (charges no longer
// name their invoice directly -- InvoicePayments link the two).
async function invoiceIdForPaymentIntent(paymentIntentId) {
 const payments = await getStripe().invoicePayments.list({
  payment: { type: 'payment_intent', payment_intent: paymentIntentId },
  status: 'paid',
  limit: 1,
 });
 return payments.data.length ? idOf(payments.data[0].invoice) : null;
}

const dashboardUrl = (livemode, path) => `https://dashboard.stripe.com/${livemode ? '' : 'test/'}${path}`;
const receiptNumberOf = (row) => `${row.number}/${row.premises}/${row.device}`;

// refund.created / refund.updated: every refund of a fiscalized payment gets
// its storno receipt (see lib/atodo/fiscal/receipts.js), issued as soon as
// Stripe accepts the refund ('pending' or 'succeeded') -- once per refund.
// Throws (→ Stripe re-delivers) while the payment's own receipt doesn't
// exist yet or fiscalization isn't configured. A refund that then fails
// after its storno went out is left for the owner to sort out by hand.
async function handleRefund(refund) {
 const { rows: issued } = await db.query('SELECT * FROM atodo.fiscal_receipts WHERE stripe_refund_id = $1', [refund.id]);
 if (['failed', 'canceled'].includes(refund.status)) {
  if (issued.length) {
   await notifyOwner(
    'a refund failed after its storno receipt was issued',
    [`Stripe refund ${refund.id} ended up ${refund.status}${refund.failure_reason ? ` (${refund.failure_reason})` : ''}, but storno receipt ${receiptNumberOf(issued[0])} was already issued and fiscalized for it. The customer hasn't got their money back, yet the storno stands (a fiscalized receipt can't be withdrawn): settle it by hand -- e.g. pay the refund outside Stripe, or issue a correcting receipt. A new refund in Stripe would get a second storno receipt of its own.`],
    [['Refund', refund.id], ['Amount', `${(refund.amount / 100).toFixed(2)} ${String(refund.currency).toUpperCase()}`], ['Storno receipt', receiptNumberOf(issued[0])]],
    { label: 'Open in Stripe', url: dashboardUrl(refund.livemode, `payments/${idOf(refund.payment_intent) || idOf(refund.charge)}`) }
   );
  }
  return;
 }
 if (!['pending', 'succeeded'].includes(refund.status)) return; // e.g. requires_action -- a later refund.updated settles it
 if (issued.length) return;
 if (skippingReceipts()) {
  console.log(`[atodo billing] refund ${refund.id} (Stripe test mode) -- no storno receipt issued`);
  return;
 }

 const paymentIntentId = idOf(refund.payment_intent);
 const invoiceId = paymentIntentId && (await invoiceIdForPaymentIntent(paymentIntentId));
 if (!invoiceId) {
  console.error(`[atodo billing] refund ${refund.id} isn't for a subscription invoice -- no storno receipt issued`);
  return;
 }
 const { rows: [original] } = await db.query(
  'SELECT * FROM atodo.fiscal_receipts WHERE stripe_invoice_id = $1 AND stripe_refund_id IS NULL',
  [invoiceId]
 );
 if (!original) throw new Error(`refund ${refund.id}: invoice ${invoiceId} has no receipt yet -- storno will be retried`);
 if (refund.currency !== 'eur') throw new Error(`refund ${refund.id} is in ${refund.currency}, not EUR -- can't be fiscalized`);
 const config = getFiscalConfig();
 if (!config) throw new Error(`refund ${refund.id}, but fiscalization isn't configured -- will be retried`);
 ...
 await issueReceipt(config, {
  ...
  description: `Storno računa br. ${receiptNumberOf(original)}: ${original.description}`,
  totalCents: -refund.amount,
 });
}

// charge.dispute.created: a chargeback. Stripe has already taken the amount
// (plus its fee) back; answering it -- submitting evidence before the
// deadline, or accepting it -- is for a person, in the Stripe Dashboard, so
// the owner gets an email with everything needed to find it.
async function handleDisputeCreated(dispute) {
 let customer = null;
 try {
  const charge = await getStripe().charges.retrieve(idOf(dispute.charge));
  const account = await findAccount({ customerId: idOf(charge.customer) });
  customer = (account && account.email) || (charge.billing_details && charge.billing_details.email) || null;
 } catch (err) {
  console.error(`[atodo billing] dispute ${dispute.id}: couldn't look up its charge: ${err.message}`);
 }
 const dueBy = dispute.evidence_details && dispute.evidence_details.due_by;
 await notifyOwner(
  'a payment was disputed',
  [
   'A customer disputed a payment with their bank. The amount (and the dispute fee) has been taken back; respond in the Stripe Dashboard -- submit evidence, or accept the dispute -- before the deadline.',
   'If the dispute is lost, the payment is effectively refunded and its receipt needs a storno, issued by hand (a dispute is not a Stripe refund, so none is issued automatically).',
  ],
  [
   ['Dispute', dispute.id],
   ...
   ['Customer', customer || '-'],
   ['Evidence due by', dueBy ? new Date(dueBy * 1000).toISOString() : '-'],
  ],
  { label: 'Open in Stripe', url: dashboardUrl(dispute.livemode, `disputes/${dispute.id}`) }
 );
}
